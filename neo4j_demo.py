...
def analyze_connectivity(tx):
    ...
    RETURN n.name AS node, degree, labels(n)[0] AS type
...
def connect_researchers_to_domains(tx):
  ...
  CREATE (r)-[:WORKS_IN]->(d)
  with r
  MATCH (r1:Researcher {name: 'Yann LeCun'}), (dl:Technology {name: 'DeepLearning'})
  CREATE (r1)-[:CONTRIBUTED_TO]->(dl)
  with r1
  MATCH (r2:Researcher {name: 'Andrew Ng'}), (ml:Technology {name: 'MachineLearning'})
  CREATE (r2)-[:CONTRIBUTED_TO]->(ml)
  """
  tx.run(query)
